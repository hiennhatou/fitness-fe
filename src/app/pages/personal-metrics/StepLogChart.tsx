...
export default function StepLogChart() {
  const [isLoading, setIsLoading] = useState(false);
  const [dailySteps, setDailySteps] = useState<[string, number][]>([]);

  const chartSeries = [
    {
      name: "Số bước",
      data: dailySteps.map(([, steps]) => steps),
    },
  ];

  const chartOptions = useMemo(() => getChartOptions(dailySteps.map(([date]) => dayjs(date, "DD:MM:YYYY").format("DD/MM"))), [dailySteps]);

  const fetchDailySteps = async () => {
    setIsLoading(true);
    try {
      const { data } = await secureApi.get<[string, number][]>("/step-log/daily");

      const stepsMap = new Map(data);
      ...
      const filledData = last7Days.map((date) => [date, stepsMap.get(date) || 0] as [string, number]);

      setDailySteps(filledData);
    } catch (error) {
      console.error(error);
