...
export function ExerciseCard({ exercise }: { exercise: IExercise }) {
  ...
  return (
    <div
      ...
      className={`cursor-pointer rounded-xl relative h-60 overflow-hidden border-[0.1rem] border-gray-400 [&.active]:border-2 [&.active]:border-[#9cd3ff] [&.active_.start-session-button]:inline-block [&.active_.img-wrapper]:relative [&.active_.img-wrapper]:h-70 [&.active]:exerciseopen [&.] [&.active_.content-wrapper_p]:line-clamp-none [&.active_.content-wrapper]:backdrop-blur-none [&.active_.content-wrapper]:text-shadow-none [&.active_.content-wrapper]:bg-[rgb(231,244,255)] [&.inactive]:exerciseclose ${isActive ? "active" : isActive === false ? "inactive" : ""}`}>
      ...
      <div className="content-wrapper text-blue-500 relative z-20 w-full h-full px-3 py-2">
        ...
        <Link to={`/start-session?exercise_id=${exercise.id}`} className="bg-blue-400 text-white px-4 py-2 rounded-xl my-3 hidden start-session-button">
          Bắt đầu phiên tập
        </Link>
      </div>
    </div>
